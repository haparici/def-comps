...
var meaning = function(description, granularity, context, referent) {
  ...
  var comparative = function() {
    ...
    var NPTrueStrictFn = function() {
      ...
      // In my case, it's just that an imprecision grouping of 2 is used, or granularity = 1 (one bar) and
      // that the referent in question is in the highest imprecision group.
      var comparativeTrue = context.length == 2 && isInContext(context, referent) && context[context.length - 1] == referent && context[0][1] != context[1][1];
      return nounTrue && comparativeTrue;
    }
    return [NPTrueStrictFn(), NPTrueFn() && DPTrueFn()];
  }
  ...
  var superlative = function() {
    ...
    return [NPTrueFn() && DPTrueFn(), NPTrueFn() && DPTrueFn()];
  }
...
var literalListener = function(description, granularity, context) {
  return Infer({method: "enumerate"}, function() {
    ...
    var truths = meaning(description, granularity, context, referent)
    
//     if (referent == "none") {
//       var check = map(function(x) { 
//         var m = meaning(description, granularity, context, x) 
//         return (m[0] || m[1]) ? 1 : 0;
//       }, context)
//       condition(!find(function(x) { x == 1 }, check));
//     }
    
    condition(truths[0] || truths[1]);
    return {"r1": truths[0] ? referent : null, "r2": truths[1] ? referent : null};
  })
}
...
var speaker = function(context, granularity, referent) {
  return Infer({method: "enumerate"}, function() {
    ...
    var LLModule = literalListener(description, granularity, context);
    var strong = LLModule.score({"r1": referent, "r2": referent});
    // when the strong and weak interpretations both lead to valid answers.
    // whenever strong is NOT POSSIBLE, we want to penalise the speaker some amount.
    factor(rationality * (strong == -Infinity ? 0 : strong)); // this is where we would put the penalty for weakening
    
    // problem: this term is -inf if this doesn't exist in the literalListener posterior.
    // this in turn forces the entire score of the speaker output distribution to be -inf,
    // or zero probability.
    var weakened = LLModule.score({"r1": null, "r2": referent});
    factor(rationality * (weakened == -Infinity ? 0 : weakened));
    
    
    factor(rationality * (strong == -Infinity && weakened == -Infinity ? -Infinity : 0));
