...
var referentsPrior = function(context) {
  ...
  var mass = (100 * context.length) * failProbability / (1 - failProbability);
  var probabilities = map(function(n) {return n == "none" ? mass : 100; }, extended);
...
var speakerVisualiser = function(context, referent) {
  return Infer({method: "enumerate"}, function() {
    // Infer over descriptions--should cost-based description be used here then as well as in the pragmatic speaker?
    var description = descriptionPrior();
    ...
    return {"description": description, "granularity": [g2]};
  })
}
...
var scene = [["box", 10],["box", 12],["box", 30]];
viz.table(speakerVisualiser(scene, scene[2]));
